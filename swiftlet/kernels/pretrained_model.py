...
class AutoParameterMapper:
    ...
    def find_best_matches(self, source_keys: List[str], target_keys: List[str], 
                         source_dict: Dict, target_dict: Dict) -> Dict[str, str]:
    ...
    def handle_qkv_combination(self, source_dict: Dict, target_dict: Dict) -> Dict[str, torch.Tensor]:
...
class PreTrainedModel:
    ...
    def from_pretrained(self, model_path: str, map_location="cpu", strict=False, verbose=True):
        """
        ...
        def _smart_parameter_mapping(source_dict, target_dict):
            ...
            qkv_combinations = self.mapper.handle_qkv_combination(processed_source, target_dict)
            ...
            mappings = self.mapper.find_best_matches(source_keys, target_keys, processed_source, target_dict)
            ...
            success_rate = (mapped_count / total_source) * 100 if total_source > 0 else 0
            coverage_rate = (mapped_count / total_target) * 100 if total_target > 0 else 0
                ...
                for qkv_target_key in qkv_combinations.keys():
                    ...
                    if layer_match:
                        ...
                        for source_key in source_dict.keys():
                            if (f'layers.{layer_id}.' in source_key or f'layers.{layer_id}.' in source_key) and \
                               any(proj in source_key for proj in ['q_proj', 'k_proj', 'v_proj']):
            ...
            if verbose:
                ...
                if unmapped_source:
                    print(f"   Unmapped source keys ({len(unmapped_source)}): {list(unmapped_source)[:3]}{'...' if len(unmapped_source) > 3 else ''}")
                
                if unmapped_target:
                    print(f"   Missing target keys ({len(unmapped_target)}): {list(unmapped_target)[:3]}{'...' if len(unmapped_target) > 3 else ''}")
            ...
            try:
                ...
                if verbose:
                    ...
                    if not strict and (missing_keys or unmapped_keys):
                        if missing_keys:
                            print(f"⚠️  Missing {len(missing_keys)} parameters (using model defaults)")
                        if unmapped_keys:
                            print(f"⚠️  {len(unmapped_keys)} source parameters were not used")
        ...
        if os.path.isfile(model_path):
            ...
            source_dict = ckpt.get("model_state_dict", ckpt) if isinstance(ckpt, dict) else ckpt
