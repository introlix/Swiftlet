...
class Gemma3ForCausalLM(nn.Module):
    ...
    def from_pretrained(self, model_path: str, map_location="cpu"):
        ...
        def _map_gemma3_keys(hf_key):
            ...
            # ... (exactly your existing mapping logic) ...
            # unchanged from your version above
            return hf_key
        ...
        def _combine_qkv_weights(raw_weights):
            ...
            for key, weight in raw_weights.items():
                if "self_attn" in key and any(proj in key for proj in ["q_proj", "k_proj", "v_proj"]):
                    # extract layer_idx same as before
                    parts = key.split(".")
                    layer_idx = parts[2] if parts[0] == "model" else parts[1]
                    grp = qkv_groups.setdefault(layer_idx, {})
                    if "q_proj" in key:
                        grp['q'] = weight; print(f"Found Q proj for layer {layer_idx}: {weight.shape}")
                    elif "k_proj" in key:
                        grp['k'] = weight; print(f"Found K proj for layer {layer_idx}: {weight.shape}")
                    elif "v_proj" in key:
                        grp['v'] = weight; print(f"Found V proj for layer {layer_idx}: {weight.shape}")

            combined = 0
            for layer_idx, grp in qkv_groups.items():
                if {'q','k','v'}.issubset(grp):
                    concat = torch.cat([grp['q'], grp['k'], grp['v']], dim=0)
                    combined_weights[f"model.layers.{layer_idx}.self_attn.qkv_proj.weight"] = concat
                    combined += 1
                else:
                    print(f"⚠️ Incomplete QKV set for layer {layer_idx}: {list(grp.keys())}")

            print(f"Successfully combined QKV weights for {combined} layers")
            return combined_weights

        def _remap_keys_and_combine(raw):
            """Remap keys and handle special cases like QKV combination."""
            # ——— NEW: drop any ".linear" qualifiers upfront ———
            normalized = {}
            for k, v in raw.items():
                clean_k = k.replace(".linear.weight", ".weight")
            ...
            for k, v in raw.items():
                ...
                # separate out any individual q/k/v for later combining
                ...
                remapped[new_key if new_key else k] = v

            if unmapped_qkv:
                print(f"Processing {len(unmapped_qkv)} separate QKV weights")
                remapped.update(_combine_qkv_weights(unmapped_qkv))
        ...
        if safefiles:
            ...
            if not raw:
                raise RuntimeError(f"No tensors loaded from {model_path!r}")
            ...
            if hasattr(self, 'local_freqs_cis') and 'local_freqs_cis' not in sd:
                print("⚠️ local_freqs_cis not found, using initialized values")
            if hasattr(self, 'global_freqs_cis') and 'global_freqs_cis' not in sd:
                print("⚠️ global_freqs_cis not found, using initialized values")
